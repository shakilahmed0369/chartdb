...
export const TableField = React.memo<TableFieldProps>(
    ({ field, updateField, removeField }) => {
        ...
        const dataFieldOptions: SelectBoxOption[] = React.useMemo(
            () =>
                sortedDataTypeMap[databaseType].map((type) => ({
                    label: type.name,
                    value: type.id,
                    regex: type.hasCharMaxLength
                        ? `^${type.name}\\(\\d+\\)$`
                        : undefined,
                    extractRegex: type.hasCharMaxLength
                        ? /\((\d+)\)/
                        : undefined,
                })),
            [databaseType]
